...
class Embeddings(nn.Module):
    def __init__(self, inch=1, dims=[8, 16, 32], depths=[1, 1, 3], kernels=[3, 3, 7]):
        super(Embeddings, self).__init__()
        self.stem = nn.Sequential(
            nn.Conv3d(inch, dims[0], kernel_size=3, stride=1, padding=1, bias=True),
...
class MobileUViT(nn.Module):
    def __init__(self,
        ...
        self.head = nn.Conv3d(dims[0], out_channel, kernel_size=1, stride=1, padding=0)
...
def mobileuvit(inch=1, dims=[16, 32, 64, 128], depths=[1, 1, 3, 3, 3], kernels=[3, 3, 7], embed_dim=256, out_channel=14):
    return MobileUViT(inch=inch, dims=dims, depths=depths, kernels=kernels, embed_dim=embed_dim, out_channel=out_channel)


def mobileuvit_l(inch=1, dims=[32, 64, 128, 256], depths=[1, 1, 3, 3, 4], kernels=[3, 3, 7], embed_dim=512, out_channel=14):
    return MobileUViT(inch=inch, dims=dims, depths=depths, kernels=kernels, embed_dim=embed_dim, out_channel=out_channel)
